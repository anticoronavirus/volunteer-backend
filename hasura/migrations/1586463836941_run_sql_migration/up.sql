...
SELECT (days.date)::date AS date,
    ...
    sum(period.vdemand)::integer AS demand,
    (((sum(period.vdemand))::numeric - COALESCE(sum(vs_stat.subs), (0)::numeric)))::integer AS placesavailable,
    gen_random_uuid() as uid
    FROM (((select *, case when shortname = 'Коммунарка' then 0 else demand end as vdemand from period inner join hospital on hospital.uid = hospital_id) period
